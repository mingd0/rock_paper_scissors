...
class Game:
    ...
    def end_game(self):
        print("Game ended. Final score: Player = %s, Computer = %s"
              % (self.get_score('player'), self.get_score('computer')))
        quit()

    def game_logic(self, player_input, computer_input):
    ...
    def game_controller(self, result):
        if result == 1:
            ...
            self.print_round_result('player',
                                    self.get_score('player'),
                                    self.get_score('computer'))
        elif result == 0:
            # Tie
            self.print_round_result('tie',
                                    self.get_score('player'),
                                    self.get_score('computer'))
        elif result == -1:
            # Player loses
            self.increment_score('computer')
            self.print_round_result('computer',
                                    self.get_score('player'),
                                    self.get_score('computer'))
        elif result == -999:
            # Quit
    ...
    def play(self):
        while True:
            player_choice = input("Please enter your choice ('r/p/s'). "
                                  "Enter 'q' to quit: ")
            computer_choice = random.choice(self.CHOICES)
            if self.validatate_player_input(player_choice):
                print("You chose %s, the computer chose %s."
                      % (player_choice, computer_choice))
                # Determine who won the round
                result = self.game_logic(player_choice, computer_choice)
                # Print result and increment score appropriately
